...
/**
 * One row of the triage list (`UI.adoc` §B1).
 *
...
data class EpisodeUi(
    ...
    /** The typed failure, so a row can tell *Retry* from *Choose folder* (`architecture.adoc` §11). */
    ...
    /** Terminal rows render de-emphasised but stay fully interactive (`UI.adoc` §5). */
    val isDeEmphasised: Boolean
        get() = ledgerState in TERMINAL_STATES
}

/** Never reconstructed from a ledger row — only ever from a live update (`UI.adoc` §B7). */
...
enum class EpisodeUiAction {
    ...
    /**
     * Puts a decided episode back into *To decide* (`decisions/0024`).
     *
...
/**
 * The single source of "what can this row do", per `UI.adoc` §12.6's table.
 *
...
/**
 * Projects a stored [EpisodeListItem] into its row, applying `UI.adoc` §B7's table.
 *
...
fun EpisodeListItem.toUi(
    ...
    EpisodeUi(
        ...
        feedTitle = feedTitle,
        title = episode.title,
        // "episode image if the feed supplies one, else the feed's" (UI.adoc §5). `Episode.imageUrl`
 ...
 * resume it (`UI.adoc` §B7's third case).
 *
