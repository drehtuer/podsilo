...
/**
 * One row of the triage list (`docs/UI.adoc` §B1).
 *
...
data class EpisodeUi(
    ...
    /** The typed failure, so a row can tell *Retry* from *Choose folder* (`docs/architecture.adoc` §11). */
    ...
    /** Terminal rows render de-emphasised but stay fully interactive (`docs/UI.adoc` §5). */
    val isDeEmphasised: Boolean
        get() = ledgerState in TERMINAL_STATES
}

/** Never reconstructed from a ledger row — only ever from a live update (`docs/UI.adoc` §B7). */
...
/**
 * The single source of "what can this row do", per `docs/UI.adoc` §12.6's table.
 *
...
/**
 * Projects a stored [EpisodeListItem] into its row, applying `docs/UI.adoc` §B7's table.
 *
...
fun EpisodeListItem.toUi(
    ...
    EpisodeUi(
        ...
        feedTitle = feedTitle,
        title = episode.title,
        // "episode image if the feed supplies one, else the feed's" (docs/UI.adoc §5). `Episode.imageUrl`
 ...
 * resume it (`docs/UI.adoc` §B7's third case).
 *
