...
/**
 * S3 — the episode detail sheet (`UI.adoc` §B4).
 *
 * A **read step inside triage**, reachable for every episode regardless of state, including the
 * de-emphasised ones (`UI.adoc` §6). It carries the same [EpisodeUi] the row does — so the sheet
...
data class EpisodeDetailUiState(
    ...
    /** `null` → no browser row at all, rather than a dead tap (`UI.adoc` §6). */
...
sealed interface EpisodeDetailEffect {
     ...
     * because leaving to read show notes is not a triage decision (`UI.adoc` §6).
     */
    data class OpenUrl(
        val url: String,
    ) : EpisodeDetailEffect
    ...
    /** Deciding closes the sheet (`UI.adoc` §6); dismissal is the host's to perform. */
