...
/**
 * S4 — settings (`UI.adoc` §7). A plain scrolling list of grouped rows, reached from S1's gear.
 *
...
/**
 * Backup and restore of the local database (`UI.adoc` §7).
 *
...
private fun NextcloudGroup(
    ...
    if (nextcloud.isConnected) {
        ...
        // happen" — so "make it happen, in this direction" belongs beside it (`UI.adoc` §7).
 ...
 * what a user checking this row wants to know (`UI.adoc` §7).
 */
...
private fun AboutGroup(
    ...
    GroupHeader("ABOUT")
    // `UI.adoc` §C4.3: the horizontal lockup, flush left on the surface ground, no card and no
