...
class FakeGpodderClient(
    ...
    /** Returned as `Result.failure`, not thrown -- the port returns its failures (`GpodderClient`). */
    ...
    override suspend fun fetchSubscriptions(since: Long?): Result<SubscriptionDelta> {
        subscriptionsFailure?.let { return Result.failure(it) }
    ...
    override suspend fun fetchEpisodeActions(since: Long): Result<EpisodeActionPage> {
        ...
        episodeActionsFailure?.let { return Result.failure(it) }
        return Result.success(episodeActionsPage)
    }
}
