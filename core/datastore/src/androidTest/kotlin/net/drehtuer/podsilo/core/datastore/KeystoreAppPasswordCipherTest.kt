...
/**
 * `KeystoreAppPasswordCipher` on a real Android Keystore — the thing `docs/architecture.adoc` §2 says can
 * only be checked here.
 *
 * Robolectric has no `AndroidKeyStore` provider, so until this existed the class had **never
 * executed**: `docs/dev-environment.adoc` listed it as "never run", and the whole guarantee that the
