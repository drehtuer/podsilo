...
// The brand mark and its lockups (`docs/UI.adoc` Part C).
//
...
// nothing, is never tappable, and has exactly the placements `docs/UI.adoc` §C4 lists. Putting it in
...
/** S1's app bar (`docs/UI.adoc` §C4.1). 24 dp beside the title, which stays live type. */
val MarkSizeAppBar = 24.dp

/** The About row's horizontal lockup (`docs/UI.adoc` §C4.3). */
val MarkSizeLockupHorizontal = 36.dp

/** S1's not-configured empty state (`docs/UI.adoc` §C4.2) — the one large, unhurried appearance. */
val MarkSizeLockupStacked = 56.dp

/** Mark to wordmark — 8 dp in both lockups and in S1's app bar (`docs/UI.adoc` §C4.1). */
val LogoGap = 8.dp
 ...
 * a wordmark is always beside it), so no semantics query can find it. `docs/UI.adoc` §C4 says the mark
...
/** Below this the three bars stop separating (`docs/UI.adoc` §C1) — use nothing rather than less. */
...
internal fun requireLegibleMarkSize(size: Dp) {
    require(size.value >= MIN_MARK_DP) {
        "The mark has a ${MIN_MARK_DP}dp floor (docs/UI.adoc §C1); below it the bars stop separating. Got $size"
    }
}
 ...
 * on the accent field `docs/UI.adoc` §C1 says the whole mark is white. So this picks by the *theme's*
 * surface luminance rather than by a `drawable-night` qualifier: the theme is a user preference in
 * DataStore (`docs/UI.adoc` §12.7) and can disagree with the device's night mode, and a qualifier
 ...
 * destroys the figure (`docs/UI.adoc` §C6).
 *
...
enum class LockupOrientation {
    /** Mark + wordmark on one line — the About row (`docs/UI.adoc` §C4.3). */
    HORIZONTAL,

    /** Mark over wordmark — S1's not-configured empty state (`docs/UI.adoc` §C4.2). */
    STACKED,
}
 ...
 *
 * **The wordmark is set as type, never imported as art.** `docs/UI.adoc` §C4.1 already requires this
...
fun PodsiloLockup(
    ...
    // No content description on the pair, and none on the mark inside it. `docs/UI.adoc` §C6 asks for
...
/**
 * `podsilo` — all lowercase, heavy, tight, flush left (`docs/UI.adoc` §C2).
 *
