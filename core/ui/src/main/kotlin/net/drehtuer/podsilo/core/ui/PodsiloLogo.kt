...
// The brand mark and its lockups (`docs/logo.md`).
//
...
// nothing, is never tappable, and has exactly the placements `docs/logo.md` §4 lists. Putting it in
...
/** S1's app bar (`docs/logo.md` §4.1). 24 dp beside the title, which stays live type. */
val MarkSizeAppBar = 24.dp

/** The About row's horizontal lockup (`docs/logo.md` §4.3). */
val MarkSizeLockupHorizontal = 36.dp

/** S1's not-configured empty state (`docs/logo.md` §4.2) — the one large, unhurried appearance. */
val MarkSizeLockupStacked = 56.dp

/** Mark to wordmark — 8 dp in both lockups and in S1's app bar (`docs/logo.md` §4.1). */
val LogoGap = 8.dp
 ...
 * a wordmark is always beside it), so no semantics query can find it. `docs/logo.md` §4 says the mark
...
/** Below this the three bars stop separating (`docs/logo.md` §1) — use nothing rather than less. */
...
internal fun requireLegibleMarkSize(size: Dp) {
    require(size.value >= MIN_MARK_DP) {
        "The mark has a ${MIN_MARK_DP}dp floor (docs/logo.md §1); below it the bars stop separating. Got $size"
    }
}
 ...
 * on the accent field `docs/logo.md` §1 says the whole mark is white. So this picks by the *theme's*
 ...
 * destroys the figure (`docs/logo.md` §6).
 *
...
enum class LockupOrientation {
    /** Mark + wordmark on one line — the About row (`docs/logo.md` §4.3). */
    HORIZONTAL,

    /** Mark over wordmark — S1's not-configured empty state (`docs/logo.md` §4.2). */
    STACKED,
}
 ...
 *
 * **The wordmark is set as type, never imported as art.** `docs/logo.md` §4.1 already requires this
...
fun PodsiloLockup(
    ...
    // No content description on the pair, and none on the mark inside it. `docs/logo.md` §6 asks for
...
/**
 * `podsilo` — all lowercase, heavy, tight, flush left (`docs/logo.md` §2).
 *
