 ...
 * later by reading the message text (`docs/UI.adoc` §B1).
 *
 * This exists because a screen has to make a decision that a sentence cannot support:
 * `docs/UI.adoc` §12.11 and `docs/architecture.adoc` §11 require that a row whose download failed because the
 ...
 * `docs/architecture.adoc` §4: snapshot at write time what a later reader will need).
 *
...
enum class ErrorCause {
     ...
     * field of every action posted to the shared GPodder log (`docs/architecture.adoc` §4/§6). A
