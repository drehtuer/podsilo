...
third-party badge service can read them either. xref:SECURITY.adoc[`SECURITY.md`] records what they cover.
...
xref:docs/dev-environment.adoc[`docs/dev-environment.adoc`] §6.
____
...
first pick, and xref:docs/architecture.adoc[`docs/architecture.adoc`] §7 and §11 say why -- as does the
...
Setup instructions are in xref:docs/dev-environment.adoc[`docs/dev-environment.adoc`], which is also honest
...
xref:docs/architecture.adoc[`docs/architecture.adoc`]; the whole of the UI -- the screen design, the Compose
seam it binds to, and the brand mark -- is in xref:docs/UI.adoc[`docs/UI.adoc`].

Five documents, and that is deliberate: `architecture.adoc` and `UI.adoc` are the two references,
xref:docs/backlog.adoc[`docs/backlog.adoc`] holds what is not being built,
xref:docs/third-party.adoc[`docs/third-party.adoc`] tracks licences, and
xref:docs/journal.adoc[`docs/journal.adoc`] keeps a running log -- this project doubles as an experiment in
agent-driven development, so the process is recorded alongside the result.
link:docs/decisions/[`docs/decisions/`] keeps only the decisions that still constrain the code and
...
to a reader: xref:.claude/CLAUDE.adoc[`.claude/CLAUDE.md`] is the standing brief every session starts
from -- scope, the non-goals, the mandated libraries, the definition of done -- and
xref:.claude/skills/run-podsilo/SKILL.adoc[`.claude/skills/run-podsilo/SKILL.md`] documents the driver
