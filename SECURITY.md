...
| Fix for something theoretical or low-impact | may become a `docs/backlog.md` entry with the reasoning, rather than a patch |
| Disclosure | coordinated. Please allow 90 days, or until a release goes out — whichever is sooner. |
| Credit | happily given in the advisory and `docs/journal.md`, or withheld if you prefer |
...
- **Your audio player.** Once a file is in your folder it belongs to you and the player
  (`README.md`); Podsilo does not manage, track, or delete it.
...
- **The app never sees your Nextcloud account password.** Authentication is **Login Flow v2** only:
  ...
  the UI, and adding one was declined deliberately (`docs/UI.md` §8, `docs/backlog.md`).
...
- **The error log is redacted by construction**: it never contains the app password, the Basic-auth
  header, or URLs carrying credentials (`docs/UI.md` §11). It is device-local — *Copy*/*Share* are
...
- **Dependencies are pinned** in `gradle/libs.versions.toml` (no floating versions) and every one is
  licence-reviewed in [`docs/third-party.md`](docs/third-party.md). Releases are minified R8 builds
...
- **The device test tier is not run in CI** (no hosted runner has a device), so device-only code —
  including `KeystoreAppPasswordCipher` — is verified by hand on real hardware. `README.md` says when
  that last happened.
