...
**Device test set (2026-08-11, Pixel 10a / Android 17): 60 tests, 54 passed, 0 failed, 6 skipped.**
The first run since the 2026-08-10 row changes caught what CI structurally cannot: two conformance
tests still asserting the pre-change UI shape. **Neither was an app bug** — *Choose folder* moved
into the row overflow, and S1's now-scrolling chip row made `hasScrollAction()` ambiguous. Both are
fixed; the second fix is a copy of one the Robolectric suite already carried, which is the clearest
sign yet that a device test can rot for as long as nobody runs it. The 6 remaining skips are the
long-standing `SafDownloadTargetInstrumentedTest` opt-out on an install with no SAF grant — the SAF
write is the one thing the set still does not actually exercise.
