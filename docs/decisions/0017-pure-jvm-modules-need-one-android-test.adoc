...
* Tier 2 stops being purely a convenience. `docs/dev-environment.adoc` §6 already says it is minutes
...
* CI still runs no emulator job (`docs/dev-environment.adoc` §6). This test therefore runs when someone
runs it, which is honest rather than satisfying; a CI emulator job is in `docs/backlog.adoc`.
