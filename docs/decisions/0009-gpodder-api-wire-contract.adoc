...
Accepted. Resolves `docs/architecture.adoc` §12 open decision #2 (subscriptions `add`/`remove`
response shape).
...
`Z` -- and `docs/architecture.adoc` §6 is updated accordingly. Podsilo still _emits_ the bare form, which
