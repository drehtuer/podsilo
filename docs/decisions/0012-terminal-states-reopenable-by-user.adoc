...
*Accepted 2026-08-01.* Drafted from the UI design work (`docs/UI.adoc` §B8.2,
`docs/UI.adoc` §12.3/§14.1); the four points it left open were settled by the author on that date and
...
Resolves the ADR `docs/UI.adoc` §14.1 asks for.

== Context

`docs/architecture.adoc` §9 originally stated that `DOWNLOADED`, `SKIPPED` and `HANDLED_REMOTELY` are
...
* *There is no undo* (`docs/UI.adoc` §12.3). A wrong decision is corrected by acting again, not by
racing a snackbar timer. If terminal states cannot be re-opened, a mis-swipe is permanent.
* `HANDLED_REMOTELY` means _another client_ decided. The user must be able to override that -- the
episode was never theirs to decide on until now.
* `SKIPPED` covers the bulk "mark old episodes as played" write (`docs/UI.adoc` §7), which is a
deliberately coarse operation. Its safeguard is a counted preview _plus_ the ability to still
...
item 6's no-auto-download invariant _structural_ rather than a matter of care. Removing it to make
_Download again_ work would trade a proven invariant for a UI affordance.
...
would make a fresh download render as _attempt 3 of 3_ in S7 and immediately look exhausted.
`lastError` is cleared at the same time.

=== 3a. _Mark as played_ over a terminal row follows the identical rules
...
`KEY_USER_REQUESTED` to unlock. The flag guards _file creation_; this writes only the ledger.
...
§4's guard -- a later _Download again_ would have no target to check and would write a second copy of
...
`writtenFileName`, `EpisodeDownloader` asks `DownloadTarget.existingNames(folder)` whether _this
episode's own previously written file_ is still there. If it is, the download is *aborted, not
overwritten and not suffixed*: the ledger returns to `DOWNLOADED` unchanged and the UI reports
_"Already in your folder -- ``<name>``"_ as a snackbar and in the row/detail status line. This is an
...
`docs/architecture.adoc` §11 calls "never use `writtenFileName` as an existence check" its single most
...
suffixing is untouched and keeps doing what architecture §11 says it is for -- stopping two _different_
...
reason the row existed, and an episode that becomes new again would reappear in _To decide_ on
...
* The UI must render that third outcome. `docs/UI.adoc` §B13.s `SnackbarText.AlreadyInFolder(fileName)`
exists for it.
...
. *Re-posting `DOWNLOAD` on a re-download: yes* (§2 stands). Consistent with a first download,
...
. *`attempts` resets: yes* (§3 stands).
. *_Mark as played_ uses the same rules* -- new §3a. No flag (it writes no file), but the same
fresh row, the same re-posted action, the same `attempts` reset -- and `writtenFileName` survives,
which is the non-obvious part.
. *The "already in your folder" outcome is not counted anywhere.* It stays an informational
...
*S7's _Retry_ on an `ERROR` row* needs no flag -- `ERROR → QUEUED` was always a legal edge and
`ERROR` is not a terminal state. It does reset `attempts` like everything else, so _Retry_ and
_Download again_ leave indistinguishable ledger rows. Accepted: the ledger records what the user
