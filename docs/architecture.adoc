...
| The shared Compose vocabulary: the icon allow-list (`docs/UI.adoc` §18) and the spacing invariants (§17). No state, no ports, no screens -- it exists so those cannot drift between modules.
...
podcast app's schema. The fifth, `ERROR_LOG`, was added by the UI design (`docs/UI.adoc` §11's error
...
| Yes
| RSS `<item><link>`
| The episode's own page, for the UI's _Open in browser_ affordance (`docs/UI.adoc` §6). `null` for feeds that omit it -- the affordance is then absent, never a dead tap. *Not* derivable from `enclosureUrl`, which points at an audio file. Added in schema v2.
...
The *user* may re-open any of the three via _Download again_ (`docs/UI.adoc` §12.3), which is the
...
need it built from source or vendored (`docs/backlog.adoc`).
...
into §11, the module rules into §2, and the UI-facing ones into `docs/UI.adoc`. The same commit
...
bundled ktlint versions on an annotated constructor. See `docs/dev-environment.adoc` §8.6.
...
`docs/dev-environment.adoc` §10 is the register of what has actually been executed and where.
...
| ✅ done
| `docs/dev-environment.adoc`, `docs/journal.adoc`
...
| <<3-data-flow,§3>>, <<8-external-interface-storage-access-framework,§8>>, `docs/UI.adoc` (Parts A and B)
...
phone. Every port in `docs/UI.adoc` §B8 is implemented, not just declared.

`:core:ui` joined the module list with the icon allow-list (`docs/UI.adoc` §18) and the spacing
...
in one place: `docs/dev-environment.adoc` §1, and deliberately not restated here.* Two registers of
