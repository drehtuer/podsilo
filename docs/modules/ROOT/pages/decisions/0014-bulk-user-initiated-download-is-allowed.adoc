...
*Accepted 2026-08-01.* Resolves the ADR `UI.adoc` §14.3 asks for. Amends the wording of
...
(`UI.adoc` §5). Left unrecorded, a future reader finds a stated non-goal and a shipped feature
...
* The ledger writes go through `upsertAll` in one transaction (`UI.adoc` §B8.6), not one
`upsert` per episode.
