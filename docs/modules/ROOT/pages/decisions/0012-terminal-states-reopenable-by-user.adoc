...
*Accepted 2026-08-01.* Drafted from the UI design work (`UI.adoc` §B8.2,
`UI.adoc` §12.3/§14.1); the four points it left open were settled by the author on that date and
...
Resolves the ADR `UI.adoc` §14.1 asks for.

== Context

`architecture.adoc` §9 originally stated that `DOWNLOADED`, `SKIPPED` and `HANDLED_REMOTELY` are
...
* *There is no undo* (`UI.adoc` §12.3). A wrong decision is corrected by acting again, not by
...
* `SKIPPED` covers the bulk "mark old episodes as played" write (`UI.adoc` §7), which is a
...
`architecture.adoc` §11 calls "never use `writtenFileName` as an existence check" its single most
...
* The UI must render that third outcome. `UI.adoc` §B13.s `SnackbarText.AlreadyInFolder(fileName)`
exists for it.
