...
*Accepted 2026-08-01.* Resolves the ADR `UI.adoc` §14.2 asks for. *This changes a rule in
...
. *A read-time filter* (CLAUDE.md §5, `architecture.adoc` §4/§5). "New" means _no ledger row
...
. *A written state* (`UI.adoc` §7/§12.5/§14.2). S4's _Mark old episodes as played_ upserts
...
behind the counted preview dialog (`UI.adoc` §7), never silently at first sync.
...
* `architecture.adoc` §4/§5's description of the "New" filter is amended; `firstSeenAt` keeps its
