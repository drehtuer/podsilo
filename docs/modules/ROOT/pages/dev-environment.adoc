...
| ❌ *Never run*
| The `deploy` job is skipped on pull requests by design, so `deploy-pages` and the repository's Pages-source setting are reasoned about, not observed. First push to `main` is what proves them
|===
