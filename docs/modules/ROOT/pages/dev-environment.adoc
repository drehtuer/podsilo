...
| ✅ *Verified*
| 2026-08-23, first push to `main` after #107: `deploy` green in 10s and the site answers at https://drehtuer.github.io/podsilo[drehtuer.github.io/podsilo] -- 20 pages in the sitemap, and the diagrams are served as site assets, so nothing points at the build-time Kroki
|===
...
# 4. Build the documentation site (optional; not a CI step).
antora --fetch antora-playbook.yml && echo "open build/site/index.html"
----
...
antora --fetch --log-failure-level=warn antora-playbook.yml   # -> build/site/index.html
----

`--log-failure-level=warn` is not optional decoration: Antora reports an unresolved `xref:`, a page
...
https://drehtuer.github.io/podsilo[drehtuer.github.io/podsilo]. Pages' own build step only knows
...
* https://github.com/drehtuer/podsilo/security/policy[`SECURITY.md`] -- the build and release path
