...
[source,bash]
----
antora --fetch antora-playbook.yml     # -> build/site/index.html
----
...
* *Mermaid renders on GitHub and not on the site.* GitHub draws `[source,mermaid]` natively; Antora's
  stock UI shows it as source. Fixing that needs either `asciidoctor-kroki` (a Kroki server, public or
  self-hosted) or a supplemental UI that loads `mermaid.js` client-side. Neither is wired up.
