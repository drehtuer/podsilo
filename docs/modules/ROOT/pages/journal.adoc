...
*The deploy half is still unrun*, because it is skipped on pull requests by design. `deploy-pages` and
the repository's Pages-source setting are reasoned about rather than observed, and the first push to
`main` is what will say. `dev-environment.adoc` §1 carries both rows.
