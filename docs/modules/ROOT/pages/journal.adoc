...
Locally green from the rebuilt container: 22 pages, all 26 diagrams, no warnings. *The workflow itself
has never run* — it cannot until it is on `main`, so its service pair and the `deploy-pages` half are
reasoned about rather than observed. `dev-environment.adoc` §1 says so in the status table, and it is
the row to update once a first deployment either works or does not.
