...
CLAUDE.md's build order (§10) for _when_ to build each piece; use this document for _what_ to build
and _how the pieces fit together_. The checklist at the end cross-references both.

Diagrams use https://mermaid.js.org/[Mermaid] in `[source,mermaid]` blocks. That spelling is
deliberate and not the AsciiDoc standard one: GitHub renders Mermaid inside `.adoc` only from
`[source,mermaid]`, while IntelliJ, GitLab and `asciidoctor-diagram` expect a bare `[mermaid]` block.
GitHub is where these are read, so GitHub's spelling wins; a generated site would need
`asciidoctor-diagram` or Kroki to draw them.
...
. <<5-domain-model-repository-ports,Domain model & repository ports>>
. <<6-external-interface-nextcloud-gpodder-api,External interface: Nextcloud GPodder API>>
. <<7-external-interface-podcast-rssatom-feeds,External interface: podcast RSS/Atom feeds>>
. <<8-external-interface-storage-access-framework,External interface: Storage Access Framework>>
. <<9-episode-ledger-state-machine,Episode ledger state machine>>
. <<10-key-flows,Key flows>>
. <<11-naming-tagging-pipeline,Naming & tagging pipeline>>
. <<12-decision-record,Decision record>>
. <<13-build-order-checklist,Build-order checklist>>
...
| `:core:model`
| JVM
| Domain types (`Feed`, `Episode`, `EpisodeLedgerRow`, ...), repository/client *interfaces*, `LedgerState` enum, `SyncOutcome` sealed result types. Zero dependencies beyond Kotlin stdlib + coroutines-core (for `Flow`).

| `:core:naming`
| JVM
| Template resolution, sanitisation, truncation, collision suffixing (CLAUDE.md §6). Depends on `:core:model` for `Feed`/`Episode`.

| `:core:sync`
| JVM
| Reconciliation logic + `SyncOrchestrator` (order-of-operations per CLAUDE.md §5). Depends on `:core:model` only -- receives repository/client implementations via constructor injection.

| `:core:database`
| Android library
| Room entities, DAOs, migrations; *implements* `FeedRepository`, `EpisodeRepository`, `EpisodeLedgerRepository`, `SyncStateRepository` from `:core:model`.

| `:core:datastore`
| Android library
| Settings storage (Nextcloud URL/credentials, folder URI, sync interval, naming templates) via Jetpack DataStore + Keystore-backed encryption for the app password. Implements `SettingsRepository`.

| `:core:feed`
| Android library
| Wraps rssparser, not Stalla (§7); fetches + parses feed XML into `Feed`/`Episode`. Hosts `FeedRefreshWorker`.

| `:core:gpodder`
| *JVM* (converted from Android library -- §2)
| Retrofit client for the three GPodder endpoints Podsilo calls; *implements* `GpodderClient` from `:core:model`.

| `:core:download`
| Android library
| Download queue (`DownloadWorker`), cache→tag→SAF-copy pipeline, WorkManager state. Depends on `:core:model`, `:core:naming`.

| `:core:ui`
| Android library (Compose)
| The shared Compose vocabulary: the icon allow-list (`UI.adoc` §18) and the spacing invariants (§17). No state, no ports, no screens -- it exists so those cannot drift between modules.

| `:feature:episodes`
| Android library (Compose)
| S1 (podcast list), S2 (episode list), S3 (detail sheet) -- the triage surface. Depends on `:core:model` (ports only) and `:core:ui`; view models are built by ``:app``'s factory from its graph.

| `:feature:settings`
| Android library (Compose)
| S4 (settings), S5 (Nextcloud connection dialog), S6 (naming editor with a live preview over the real `NamingTemplateEngine`).

| `:app`
| Android application
| Hilt wiring (binds every port to its adapter), the `NavHost`, S7 (activity) and S8 (error log) -- both cross-cutting rather than episode-list concerns -- `SyncWorker` (see below), and app-level `WorkManager` scheduling.
|===
...
podcast app's schema. The fifth, `ERROR_LOG`, was added by the UI design (`UI.adoc` §11's error
...
| `String`
| No (PK)
| GPodder `subscriptions.add[i]`
| Also the value written into `EpisodeAction.podcast` on outbound actions.

| `title`
| `String`
| No
| RSS `<channel><title>`
| GPodder API has no titles -- only known after the first successful feed fetch; use the URL as a placeholder until then.

| `imageUrl`
| `String`
| Yes
| RSS `<itunes:image>` / `<image><url>`
| Stored with `http://` upgraded to `https://` (§7). Artwork only -- never the enclosure URL, which is identity.

| `firstSeenAt`
| `Long`
| No
| Local clock, set once when the URL first appears in `add[]`
| *No longer a query predicate* (ADR 0013): the "New" filter is `no ledger row`, full stop. It is now the default cutoff date offered for a newly-appearing feed in S4's _Mark old episodes as played_. Never updated after first write.

| `lastRefreshedAt`
| `Long`
| Yes
| Local clock, after a successful (200, not 304) feed fetch
|
...
| No (PK)
| `guid ?: enclosureUrl`
| Mirrors the server's identification rule exactly (CLAUDE.md §5) -- this is not a free choice.
...
| Yes
| RSS `<guid>`
| Frequently missing, reused, or changed -- see <<12-decision-record,§12>>.

| `enclosureUrl`
| `String`
| No
| RSS `<enclosure url="">`
| Used as `episodeKey` fallback and as `EpisodeAction.episode`.

| `link`
| `String`
| Yes
| RSS `<item><link>`
| The episode's own page, for the UI's _Open in browser_ affordance (`UI.adoc` §6). `null` for feeds that omit it -- the affordance is then absent, never a dead tap. *Not* derivable from `enclosureUrl`, which points at an audio file. Added in schema v2.

| `title`
| `String`
| No
| RSS `<title>`
| Stored *raw*; cleanup regex rules (§6) and sanitisation apply at naming time, not storage time.

| `description`
| `String`
| Yes
| RSS `<description>` or `<content:encoded>`
| Stored raw (CDATA and all); HTML sanitised at render time in the UI, never at write time.

| `pubDate`
| `Long`
| Yes
| RSS `<pubDate>`, fallback chain in §6
| Normalised to the device's fixed timezone (document which one is chosen when `:core:naming` lands).

| `durationMs`
| `Long`
| Yes
| RSS `<itunes:duration>`
| Notoriously unreliable -- never block anything on this being present.
|===
...
| No (PK)
| --
| Shares key space with `Episode.episodeKey`; see FK note above. `guid` is *not* a separate column -- it's derived (`episodeKey.takeIf { it != enclosureUrl }`).

| `feedUrl`
| `String`
| No
| Snapshot of `Episode.feedUrl` at write time
| Denormalised, not looked up via `Episode`. Needed so a POST retry after the feed is unsubscribed can still build a valid outbound action.

| `enclosureUrl`
| `String`
| No
| Snapshot of `Episode.enclosureUrl` at write time
| Same rationale as `feedUrl`.

| `state`
| `String`/enum
| No
| UI (QUEUED/SKIPPED), `DownloadWorker` (DOWNLOADING/DOWNLOADED/ERROR), `SyncOrchestrator` (HANDLED_REMOTELY)
| See <<9-episode-ledger-state-machine,§9>> for the full transition table. There is *no persisted "NEW" value* -- new means no row exists at all.

| `actionedAt`
| `Long`
| No
| Whoever writes `state`
| Epoch millis. For rows created from a remote action, parsed from the action's ISO-8601 `timestamp` field, interpreted as UTC (§6).

| `syncedToServer`
| `Boolean`
| No
| `false` on local write, `true` only on confirmed 2xx POST
| This is literally the outbox flag -- `getUnsynced()` is `WHERE syncedToServer = 0`.

| `attempts`
| `Int`
| No
| `DownloadWorker` / outbox push
| Retry counter.
...
| Yes
| `DownloadWorker` / outbox push
| Human-readable, for the error state UI.

| `writtenFileName`
| `String`
| Yes
| `DownloadWorker`, after a successful SAF copy
| Retry idempotency *only* -- never used as an existence check (CLAUDE.md §11's single most important invariant).

| `durationSeconds`
| `Int`
| Yes
| Snapshot of `Episode.durationMs` (converted to seconds) at write time
| Used only to encode a skip's `PLAY` `total`/`position` (§6). `null` when the feed never supplied a usable duration.
|===
...
| No (PK)
| Hardcoded `1`
| Not a real multi-row table; Room needs a PK regardless.

| `lastEpisodeActionSyncTs`
| `Long`
| No
| Verbatim from the GPodder `episode_action` response's top-level `timestamp`
| *Unix seconds*, sent back as the next `since`. Never computed from local device time -- see CLAUDE.md §11's clock-skew gotcha.

| `deviceId`
| `String`
| No
| Generated once (e.g. `UUID.randomUUID()`) on first run, persisted forever
| Lets us recognise our own echoed-back actions in the remote action stream.
|===
...
// Every stored timestamp above is a Long. The UI renders java.time values (docs/UI.adoc
...
interface EpisodeLedgerRepository {
    ...
    // Bulk triage (docs/UI.adoc §7's "mark old/all as played"): one transaction and one Flow
...
// never inferred from a timeout — so a pull-to-refresh with no network fails instantly (docs/UI.adoc §12.10).
...
interface NextcloudLoginFlowClient {
    ...
    // the caller's job at that moment is to store nothing, so nothing waits for this (docs/UI.adoc §8).
...
| GET
| `since` (Unix seconds, optional)
| `{add: [url], remove: [url], timestamp}`

| `/index.php/apps/gpoddersync/episode_action`
| GET
| `since` (Unix seconds, optional)
| `{actions: [EpisodeAction], timestamp}`
...
| [.line-through]#`/index.php/apps/gpoddersync/subscription_change/create`#
| --
| *never called*
| --
|===
...
| `since` query param (both endpoints)
| Unix seconds, `Long`
| `1752480000`

| Top-level `timestamp` in both responses
| Unix seconds, `Long`
| `1752483600` -- persist verbatim, send back as next `since`

| `EpisodeAction.timestamp` field (inside each action)
...
`decisions/0009` for the full verified contract and `0003` for the amended timestamp
decision.
____
...
Retrofit's `HttpException` propagate -- which is _not_ an `IOException`, so an expired app password
...
Nextcloud. Podsilo emits it regardless -- see `decisions/0008` before designing anything that
...
| `0`
| equal to `total`
| `EpisodeLedgerRow.durationSeconds` if known, else *`1`*

| `DOWNLOADED`
| *also* `PLAY`, posted after the `DOWNLOAD`
| `0`
| equal to `total`
| as above -- `decisions/0023`
|===
...
*The `1` is not a duration* (`decisions/0022`, amended 2026-08-14 -- this table said `0` until
then). A reader decides _played_ from `position > 0 && total > 0 && position >= total`, so a `0/0`
...
| `DOWNLOAD` or `DELETE` for an episode we don't have a terminal local state for
| `state = HANDLED_REMOTELY` -- do not download it here

| `PLAY` *that reads as ended* -- `position > 0 && total > 0 && position >= total`
| as above

| `PLAY` that does *not* read as ended -- including `position = 0`
| *ignored*: this is how a client says _unread_

| Any action for an episode not in any subscribed feed
| Still processed (§5 explicitly lists this as a test case) -- the ledger is keyed by episode, not by current subscription

| Our own device's echoed-back action
| Idempotent no-op (identified via `deviceId`, or simply because the local state already matches)
|===
...
*A `PLAY` is not automatically "handled"* (`decisions/0022`, 2026-08-14 -- this table said it
was until then). The API cannot delete an action and has no _unread_ type, so a client says _unread_
...
the one filed as terminal and hidden from _To decide_ for ever. The rule above is the reading client's
...
(§5), so this is the _only_ source of episode data.
...
| `Feed.imageUrl`
| `<itunes:image>` or `<image><url>`
| none → `null`; `http://` upgraded to `https://` (below)

| `Episode.guid`
| `<guid>`
| none → `null` (falls back to enclosure URL for `episodeKey`)

| `Episode.enclosureUrl`
| `<enclosure url="">`
| episode without an enclosure is not downloadable -- exclude or flag, decide during `:core:feed` implementation
...
| `Episode.pubDate`
| `<pubDate>`
| other date field the parser exposes → date first locally seen (§6)

| `Episode.durationMs`
| `<itunes:duration>`
| none → `null`, never invented

| `Episode.link`
| `<item><link>` / Atom `<link rel="alternate">`
| none → `null`
|===
...
every action posted to the shared log (§6), so an upgraded one is a _different episode_ to AntennaPod
...
cut version since, which fails CLAUDE.md's own "prefer the boring, _maintained_ option" bar, so the
...
permanent (and is _not_ an unsubscribe -- the subscription list is the server's, CLAUDE.md §1), a
...
This is the pipeline CLAUDE.md §6/§11 mandates and explains _why_: tagging libraries need a real
...
See <<11-naming-tagging-pipeline,§11>> for the same pipeline as a flowchart with the ledger write
folded in.
...
*`UNPLAYED` is the one state that means "no decision"* (`decisions/0024`). The user reached it
...
The *user* may re-open any of the three via _Download again_ (`UI.adoc` §12.3), which is the
...
this?" test; that stays the ledger (§11's central invariant). Specified in `decisions/0012`,
...
<<12-decision-record,§12>> for the rationale flagged as a decision
to confirm.
...
*"Later" is now user-initiated* (`decisions/0026`): there is no periodic sync pass, so the
next pass is a pull-to-refresh, S7's _Sync now_, one of S4's directional buttons, or the pass the
...
need it built from source or vendored (`backlog.adoc`).
...
* *The cache file is renamed before tagging.* jaudiotagger picks its reader from the _file
extension_, so tagging a `.partial` scratch file always fails ("no reader associated with this
...
`decisions/` holds the decisions that still constrain the code and cannot be re-derived from
this document -- a server limitation, a wire contract, a rule that amends CLAUDE.md, a platform trap.
Everything else this project decided is _in_ this document, in the section that governs it: an ADR
...
| xref:decisions/0008-nextcloud-gpodder-discards-download-actions.adoc[0008]
| `nextcloud-gpodder` ≥ 3.13.3 silently discards `DOWNLOAD` on POST and still returns 200 -- we emit it anyway
| §6 -- *a real limitation, not a choice*

| xref:decisions/0009-gpodder-api-wire-contract.adoc[0009]
| The verified wire contract: `add − remove`, action-name casing, the `-1` sentinel, bare-array POST, Basic auth, `since` inclusivity
| §6

| xref:decisions/0012-terminal-states-reopenable-by-user.adoc[0012]
| Terminal states re-open *only* on a UI event (`KEY_USER_REQUESTED`); a re-decision behaves exactly like a first one
| §9, §10

| xref:decisions/0013-backlog-cutoff-is-written-skipped-rows.adoc[0013]
| The backlog cutoff is *written `SKIPPED` rows*, not a read-time `firstSeenAt` filter -- amends CLAUDE.md §5
| §4, §5

| xref:decisions/0014-bulk-user-initiated-download-is-allowed.adoc[0014]
| Bulk download is allowed as a _command_, never as a _rule_ -- amends CLAUDE.md §1 and README
| §10

| xref:decisions/0017-pure-jvm-modules-need-one-android-test.adoc[0017]
| A pure-JVM module that ships in the app gets *one* test on a real Android runtime -- the JVM and ICU do not agree
| §2, `dev-environment.adoc` §6

| xref:decisions/0020-the-login-poll-runs-only-in-the-foreground.adoc[0020]
| The Login Flow v2 poll runs only while S5 is on screen -- a backgrounded process could not resolve the host at all
| `UI.adoc` §8, §B5

| xref:decisions/0022-play-is-an-ended-marker-in-both-directions.adoc[0022]
| A `PLAY` means _ended_, both ways: a duration-less skip sends `1/1`, and an inbound `PLAY` is only terminal when `position >= total > 0` -- which is how a client says _unread_
| §6 -- settles #60's two interop halves

| xref:decisions/0023-a-download-also-marks-the-episode-played.adoc[0023]
| A completed download emits `DOWNLOAD` *and* `PLAY` -- reverses CLAUDE.md §5's prohibition, because the server discards `DOWNLOAD` and the episode stayed new everywhere else
| §6, CLAUDE.md §1/§5

| xref:decisions/0024-mark-as-unplayed-is-a-state-not-a-delete.adoc[0024]
| _Mark as unplayed_ is a new `UNPLAYED` ledger state, not a row deletion -- the row outlives the decision, so the dedup authority is untouched
| §4, §9

| xref:decisions/0025-two-directional-sync-passes.adoc[0025]
| Two directional passes -- the pull is `since = 0` over the *unchanged* reconciliation, the push re-asserts every row and is chunked
| §6, `UI.adoc` §7

| xref:decisions/0026-manual-sync-only.adoc[0026]
| *No periodic sync pass* -- every pass is one the user asked for, and the schedule an older build persisted is _cancelled_ on start, not merely un-scheduled
| §6, §10, CLAUDE.md §11

| xref:decisions/0027-no-paging-3-the-list-is-already-fast-enough.adoc[0027]
| *No Paging 3* -- measured: 9,500 episodes reach the screen in 82--132 ms, so a keyed `LazyColumn` is the requirement rather than one of two options. Amends CLAUDE.md §3's table
| `UI.adoc` §B14.3
|===
...
into §11, the module rules into §2, and the UI-facing ones into `UI.adoc`. The same commit
(`7a9545a`) repointed all 95 KDoc citations, so nothing in the code still refers to a deleted number;
the mapping table that stood here until 2026-08-23 had no readers left. `+git log -- decisions/+`
...
bundled ktlint versions on an annotated constructor. See `dev-environment.adoc` §8.6.
...
| The backlog cutoff
| read-time `pubDate >= firstSeenAt` filter
| written `SKIPPED` rows
| CLAUDE.md §5, §4/§5 here

| Bulk download
| "no download all"
| no download _rules_; a scoped, confirmed _command_ is allowed
| CLAUDE.md §1, README

| UI scope
| "two destinations is the target"
| eight screens, each covering a state the app can be in
| CLAUDE.md §10 step 8
|===
...
`dev-environment.adoc` §10 is the register of what has actually been executed and where.
...
| 1
| Dev container + Gradle skeleton
| ✅ done
| `dev-environment.adoc`, `journal.adoc`

| 2
| `:core:model` + `:core:database` (+ `:core:datastore`)
| ✅ done (Tier 1 / 4a)
| <<4-database-schema,§4>>, <<5-domain-model-repository-ports,§5>>

| 3
| `:core:feed`
| ✅ done (Tier 2/3 parse + fetch, 4b refresh worker)
| <<7-external-interface-podcast-rssatom-feeds,§7>>

| 4
| `:core:naming`
| ✅ done (Tier 1)
| <<5-domain-model-repository-ports,§5>> (`NamingTemplateEngine`), <<11-naming-tagging-pipeline,§11>>

| 5
| `:core:download`
| ✅ done (Tier 4b)
| <<8-external-interface-storage-access-framework,§8>>, <<10-key-flows,§10>>, <<11-naming-tagging-pipeline,§11>>

| 6
| `:core:gpodder`
| ✅ done (Tier 3)
| <<6-external-interface-nextcloud-gpodder-api,§6>>

| 7
| `:core:sync`
| ✅ done (Tier 1, extended in 3/4b)
| <<2-module-architecture,§2>> (ports/adapters rule), <<6-external-interface-nextcloud-gpodder-api,§6>>, <<9-episode-ledger-state-machine,§9>>

| 8
| UI (`:feature:settings`, `:feature:episodes`, `:app`, `:core:ui`)
| ✅ done -- all eight screens, navigable and icon-complete
| <<3-data-flow,§3>>, <<8-external-interface-storage-access-framework,§8>>, `UI.adoc` (Parts A and B)

| 9
| Polish (error surfacing, per-feed counts)
| ✅ done -- S8 collects every failing path, S1 carries the per-feed counts, and the foreground-service notification has been displayed on a device
| <<9-episode-ledger-state-machine,§9>> (`ERROR` state), <<10-key-flows,§10>>
|===
...
phone. Every port in `UI.adoc` §B8 is implemented, not just declared.

`:core:ui` joined the module list with the icon allow-list (`UI.adoc` §18) and the spacing
invariants (§17), both of which are shared by every screen and were starting to drift.

*What has actually been _run_ -- against a real Nextcloud, real SAF, a real download -- is recorded
in one place: `dev-environment.adoc` §1, and deliberately not restated here.* Two registers of
