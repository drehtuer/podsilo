...
The companion is xref:architecture.adoc[`docs/architecture.adoc`] -- modules, schema, sync semantics.
...
Every feature in `docs/architecture.adoc` and the README was audited row by row against a screen or a
...
Three UX decisions here contradicted statements in `docs/architecture.adoc`, in CLAUDE.md, or in code
...
`docs/architecture.adoc` §9's state machine. The half that belongs to this document: _Download again_
...
stated where it binds -- Part B names the type, and `docs/architecture.adoc` names the port.
...
Composable. `docs/architecture.adoc` is what sits underneath it.
...
`docs/architecture.adoc` §5 names the ports -- so the index was removed on 2026-08-23. What cannot be
...
* `Instant` is *`java.time`*, and nothing was added to get it -- see §B1 and `docs/architecture.adoc` §5.
...
| a keyed `LazyColumn` with stable ``episodeKey``s -- *not* Paging 3, settled by measurement in `docs/decisions/0027`. The sticky headers and the fast-scroll thumb both depend on stable keys, and `animateItem` misbehaves without them.

| `writtenFileName` present but the file is gone
| the row still reads `DOWNLOADED`. Podsilo does not check, track, or care whether the file still exists -- the only permitted existence check is the pre-flight duplicate guard on an explicit re-download (`docs/decisions/0012`).
|===
