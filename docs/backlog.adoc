...
Finished items are *deleted*, not struck through: `docs/journal.adoc` is the record of what was
...
option; `docs/dev-environment.adoc` §7 records the deliberate decision not to build it. The cost is
...
in `docs/dev-environment.adoc` §6 and is entirely manual: run the set, grant the folder, pull to
...
already holds is in `docs/dev-environment.adoc` §10; these are the gaps, in the order they bite.
...
* *Outline the wordmark in the lockup SVGs.* `docs/UI.adoc` §C2: anything leaving the app -- store
...
*built* (`docs/decisions/0024`, 2026-08-14): it turned out the objection was to the _delete_, not
...
* *A two-pane tablet layout.* Not an omission: `docs/UI.adoc` §19 explains why the triage model makes
it the wrong shape. The content-width cap is the part of it worth keeping.
* *A splash screen.* `docs/UI.adoc` §C3 originally specified one -- the mono mark on `#EC3013`, via
...
and it costs a dependency. `docs/UI.adoc` §C3 no longer describes one; do not reintroduce it as polish.
